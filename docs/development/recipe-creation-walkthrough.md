...
5. `Recipe.Create`, `AddIngredient`, `AddStep`, `AssignIngredientToStep` y `EnsureComplete` protegen las invariantes que deben cumplirse con independencia del cliente utilizado. Las asociaciones apuntan a una línea concreta de ingrediente y no duplican sus cantidades.
...
La arquitectura general, los contratos HTTP y las reglas de persistencia se amplían en [Arquitectura para contribuidores](architecture.md), [API HTTP](api.md) y [Persistencia y migraciones](database.md).
